...
public final class VerifiedActionExecutor {
    ...
    private let traceStore: ExperienceStore?
    ...
    public func run(
        ...
        let preWorldState = WorldState(
            observationHash: preHash,
            planningState: prePlanningState,
            observation: preObservation,
            repositorySnapshot: preRepositorySnapshot,
            lastAction: intent
        )
        ...
        let postVerification = PostconditionVerifier().verify(
            action: intent.action,
            target: intent.targetQuery ?? intent.elementID,
            preState: preWorldState,
            postState: postWorldState,
            latencyMs: elapsedMs
        )

        let baseFailureClass = classifyFailure(intent: intent, raw: raw, verification: verification, timedOut: timedOut)
        let failureClass = baseFailureClass ?? (postVerification.passed ? nil : FailureClass(rawValue: postVerification.failureClass ?? "verification_failed"))
        let verified = raw.success && verification.status != .failed && postVerification.passed
