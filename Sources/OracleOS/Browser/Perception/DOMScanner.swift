...
//
// Architecture:
//   Oracle OS → WebSocket → Chrome CDP → DOM tree / CSS selectors
//
// Requires Chrome to be running with --remote-debugging-port=9222.
// DOMScanner gracefully handles the case where Chrome isn't running
...
public enum DOMScanner {
    ...
    private static func evaluateJSRaw(
                        ...
                        if let data = text.data(using: .utf8),
                           ...
                           let resultObj = json["result"] as? [String: Any] {
                            ...
                            if let exceptionDetails = resultObj["exceptionDetails"] as? [String: Any] {
                                let error = NSError(
                                    domain: "DOMScanner.RuntimeEvaluate",
                                    code: 1,
        ...
        if waitResult == .timedOut {
            fputs("DOMScanner.evaluateJSRaw: WebSocket command timed out after \(wsTimeout)s\n", stderr)
            return nil
        }
        return box.result
    }
